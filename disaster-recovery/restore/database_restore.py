...
environments = {
    "development": "705467933182",
    "preproduction": "540070264006",
    "backup": "238302996107",
}

key_alias_remote = "alias/serve-ca-db-backup"
key_alias_local = "alias/serve_rds_encryption_key"
...
class SnapshotManagement:
    def __init__(
        ...
        self.role = "serve-ci-boundary" if os.getenv("CI") else "breakglass"
        self.backup_role = (
        ...
        self.DBClusterParameterGroup = "default.aurora-postgresql14"
        ...
        self.TagList = [
            ...
            {"Key": "application", "Value": "Serve-OPG"},
    ...
    def restore(self):
        if self.account is None:
            ...
            sys.exit(1)
        self.create_serve_client_session()
        ...
        if self.AWSBackup:
            ...
            if not self.do_point_in_time_restore:
                if self.SnapshotIdentifier is not None:
                    if self.restore_from_remote:
                        print("Attempting Restore From Remote")
                        self.create_backup_client_session()
                        self.KmsKeyId = self.get_kms_key(key_alias_remote)
                        self.share_snapshot_with_serve()
                        self.copy_snapshot_to_manual_serve()
                    print("Attempting Restore From Snapshot")
    ...
    def share_snapshot_with_serve(self):
        print(
    ...
    def copy_snapshot_to_manual_serve(self):
    ...
    def create_serve_client_session(self):
        session_credentials = RefreshableCredentials.create_from_metadata(
            metadata=self.refresh_creds_serve(),
            refresh_using=self.refresh_creds_serve,
