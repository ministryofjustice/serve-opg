...
class FixtureTestHelper
{
    ...
    public function __construct(string $yamlFixtureLocation, UserPasswordHasherInterface $hasher, EntityManagerInterface $em, string $behatPassword)
    {
    ...
    public function loadCaseFixture(string $yamlFileName): void
    {
        ...
        foreach ($cases as $index => $case) {
            ...
            if (!$client->hasOrder(Order::TYPE_PF) && (Order::TYPE_BOTH == $case['type'] || Order::TYPE_PF == $case['type'])) {
            ...
            if (!$client->hasOrder(Order::TYPE_HW) && (Order::TYPE_BOTH == $case['type'] || Order::TYPE_HW == $case['type'])) {
