    ...
    public function loadCaseFixture(string $yamlFileName)
    {
        ...
        foreach ($cases as $index => $case) {
            ...
            if (!$client->hasOrder(Order::TYPE_PF) && ($case['type'] == Order::TYPE_BOTH || $case['type'] == Order::TYPE_PF)) {
                $this->em->persist(new OrderPf($client, new DateTime(rand(11, 20).' days ago'), new DateTime(rand(1, 10).' days ago'), strval($index)));
                $this->em->flush();
            }

            if (!$client->hasOrder(Order::TYPE_HW) && ($case['type'] == Order::TYPE_BOTH || $case['type'] == Order::TYPE_HW)) {
                $this->em->persist(new OrderHw($client, new DateTime(rand(11, 20).' days ago'), new DateTime(rand(1, 10).' days ago'), strval($index)));
                $this->em->flush();
            }
        }
    }
