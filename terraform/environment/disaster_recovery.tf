...
module "disaster_recovery_backup" {
  ...
  aws_subnet_ids          = local.account.use_new_network ? data.aws_subnet.application[*].id : data.aws_subnet.private[*].id
  aws_vpc_id              = local.account.use_new_network ? data.aws_vpc.main.id : data.aws_vpc.vpc.id
