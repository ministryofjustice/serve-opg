...
# Role For GitHub Actions to invoke the above Lambda

data "aws_iam_policy_document" "gh_oidc_assume" { # OpenID Connect: https://aws.amazon.com/blogs/security/use-iam-roles-to-connect-github-actions-to-actions-in-aws/
  statement {
    ...
    principals {
      type        = "Federated"
      identifiers = ["arn:aws:iam::705467933182:oidc-provider/token.actions.githubusercontent.com"]
    }
    ...
    condition {
      ...
      values   = [
        "repo:ministryofjustice/serve-opg:*"
      ]
    }
  }
}

resource "aws_iam_role" "serve_opg_slack_invoker" {
  name               = "serve-opg-slack-invoker"
  assume_role_policy = data.aws_iam_policy_document.gh_oidc_assume.json
}

data "aws_iam_policy_document" "slack_invoke" {
  statement {
    effect = "Allow"
    actions = [
    ...
    resources = [
      aws_lambda_function.serve_opg_notify_slack.arn
    ]
  }
}

resource "aws_iam_policy" "slack_invoke" {
  name   = "serve-opg-slack-invoke"
  policy = data.aws_iam_policy_document.slack_invoke.json
}

resource "aws_iam_role_policy_attachment" "slack_invoke_attach" {
  role       = aws_iam_role.serve_opg_slack_invoker.name
  policy_arn = aws_iam_policy.slack_invoke.arn
}
