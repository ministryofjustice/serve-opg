locals {
  ...
  s3_alb_log_account_name = local.s3_alb_log_account_names[local.account.name]
}

data "aws_s3_bucket" "access_logging" {
  bucket = "s3-access-logs-opg-serve-opg-${local.account.name}-${data.aws_region.current.name}"
}

resource "aws_s3_bucket" "alb_access" {
  bucket        = "alb-logs.${data.aws_region.current.name}.${local.s3_alb_log_account_name}.serve.opg.service.justice.gov.uk"
  force_destroy = local.account.name == "production" ? false : true
}
