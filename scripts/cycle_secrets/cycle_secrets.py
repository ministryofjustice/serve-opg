...
def get_session(account_id):
    ...
    if "CI" in os.environ:
        role_to_assume = f"arn:aws:iam::{account_id}:role/serve-opg-ci-boundary"
    else:
    ...
    assumed_role_object = sts_client.assume_role(
        RoleArn=role_to_assume, RoleSessionName="ServeCycleSecrets"
    )
...
def cycle_secrets(session, aws_config, secrets_list):
...
def main(environment, secret_type):
    ...
    if secret_type == "app":
        cycle_secrets(session, aws_config, app_secrets_list)
        restart_ecs_services(session, workspaces, aws_config)
    elif secret_type == "database":
        cycle_secrets(session, aws_config, db_secrets_list)
